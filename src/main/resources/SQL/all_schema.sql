...
CREATE TABLE member (
                        ...
                        mem_login_fail_count   INT        NOT NULL DEFAULT 0 COMMENT '로그인 실패 횟수',
                        mem_locked             CHAR(1)    NOT NULL DEFAULT 'N' COMMENT '계정 잠금 여부 (Y/N)',
                        mem_lock_time          DATETIME   DEFAULT NULL COMMENT '계정 잠금 시작 시간',
                        PRIMARY KEY (mem_idx),
...
CREATE TABLE bid (
                     ...
                     won_at            DATETIME      DEFAULT NULL COMMENT '낙찰 확정일시',
                     PRIMARY KEY (bid_idx),
...
CREATE TABLE review (
                        ...
                        review_keywords VARCHAR(255) NULL COMMENT 'Gemini 추출 핵심 키워드 (쉼표 구분)',
...
-- 4-2) BOARD LIKE (게시글 좋아요)
CREATE TABLE board_like (
                            board_like_idx BIGINT AUTO_INCREMENT PRIMARY KEY,
                            ...
                            UNIQUE KEY uk_board_like (board_idx, mem_idx)
);
...
-- 4-4) REPLY LIKE (댓글 좋아요)
CREATE TABLE reply_like (
                            reply_like_idx BIGINT AUTO_INCREMENT PRIMARY KEY,
                            reply_idx BIGINT NOT NULL,
                            mem_idx BIGINT NOT NULL,

                            CONSTRAINT fk_reply_like_reply
                                FOREIGN KEY (reply_idx)
                                    REFERENCES reply(reply_idx)
                                    ON DELETE CASCADE,
                            ...
                            UNIQUE KEY uk_reply_like (reply_idx, mem_idx)
);
...
CREATE TABLE payment (
                         pay_idx           BIGINT NOT NULL AUTO_INCREMENT,
                         bid_idx           BIGINT NOT NULL,
                         mem_idx           BIGINT NOT NULL,
                         ...
                         buyer_addr        VARCHAR(500) NOT NULL,
                         buyer_zipcode     VARCHAR(20) NOT NULL,

                         pay_regdate       DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    -- 추가되는 컬럼
                         ...
                         CONSTRAINT fk_payment_bid
                             FOREIGN KEY (bid_idx) REFERENCES bid(bid_idx),
...
CREATE TABLE chatroom (
                          ...
                          UNIQUE KEY ux_chatroom_unique (buyer_idx, bidder_idx),
