...
const BoardList = () => {
  ...
  /* ------------------- UI ------------------- */
  return (
    <div className="min-h-screen bg-white select-none [*_button:focus]:outline-none [*_button:active]:outline-none [*_button:focus]:ring-0 overflow-y-scroll">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 py-8">
        {/* ---------- 헤더 ---------- */}
        <header className="mb-6 pt-3 pb-3 px-3 sm:pt-10 sm:pb-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            {[{ value: "latest", label: "최신순" }, { value: "views", label: "조회수순" }].map(s => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs sm:text-sm border transition-colors focus:outline-none focus:ring-0 active:outline-none ${
                  sortType === s.value
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out lg:max-h-none lg:overflow-visible ${isAccordionOpen ? 'max-h-[1000px]' : 'max-h-0 lg:max-h-none'}`}>
                <div className="px-2 py-2">
                  <button
                    ...
                    className={`w-full inline-flex items-center rounded-lg px-4 py-3 text-sm font-semibold leading-5 transition-colors ${
                      !typeCode ? 'bg-gray-100 text-[#222222]' : 'text-[#767676] hover:bg-gray-50 hover:text-[#222222]'
                    }`}
                  >
                    전체 게시판
                  </button>
                  ...
                  {boardTypes.map(bt => (
                    <button
                      ...
                      className={`w-full inline-flex items-center rounded-lg px-4 py-3 text-sm font-semibold leading-5 transition-colors ${
                        typeCode === bt.boardTypeCode ? 'bg-gray-100 text-[#222222]' : 'text-[#767676] hover:bg-gray-50 hover:text-[#222222]'
                      }`}
                    >
                      {bt.boardTypeName}
                    </button>
            ...
            <div className="bg-white border border-gray-100 rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] overflow-hidden">
              ...
              <div className="px-6 py-5 border-b border-gray-100">
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                  <select
                    value={searchType}
                    onChange={e => updateParams({ searchType: e.target.value, page: "1" })}
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex justify-center border-t border-gray-50 pt-4">
                  <div className="flex gap-1 overflow-x-auto max-w-full pb-2 scrollbar-none select-none touch-pan-x">
                    ...
                    {Array.from({ length: blockEnd - blockStart + 1 }, (_, idx) => blockStart + idx).map(pageNum => (
                      <button
                        key={pageNum}
